...
def check_pbs_folder(directory):
    """Check if pbsfile.txt exists under <directory>/pbs/"""
    candidate = os.path.join(directory, "pbs", "pbsfile.txt")
    return candidate if os.path.isfile(candidate) else None

def find_pbs_folder(start_dir="."):
    #1. Check current dir
    #2. Check all siblings of parent dir
    current_path = os.path.abspath(start_dir)

    # Step 1: check current directory
    result = check_pbs_folder(current_path)
    if result:
        return result
    ...
    try:
        for entry in os.listdir(parent_path):
            if entry in (".", ".."):
                continue
            sibling_path = os.path.join(parent_path, entry)
            if os.path.isdir(sibling_path):
                result = check_pbs_folder(sibling_path)
                if result:
                    return result
    except Exception:
        return None
